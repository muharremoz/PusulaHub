...
import { Pencil, Key, Trash2, Info } from "lucide-react";
...
export function TabUsers({ users, localUsers }: Props) {
  ...
  const allUsers: { username: string; displayName: string; extra: string; enabled: boolean; lastLogin: string }[] = isAD
    ? users.map((u) => ({ username: u.username, displayName: u.displayName, extra: u.email || u.ou, enabled: u.enabled, lastLogin: u.lastLogin }))
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-[8px] p-2 pb-0" style={{ backgroundColor: "#F4F2F0" }}>
        <div className="rounded-[4px]" style={{ backgroundColor: "#FFFFFF", boxShadow: "0 2px 4px rgba(0,0,0,0.06)" }}>
          ...
          {/* Column headers */}
          <div className="grid grid-cols-[1fr_1fr_1fr_120px_80px] gap-3 px-3 py-2 bg-muted/10 border-b border-border/40">
            {["Kullanıcı", "Görünen Ad", isAD ? "E-posta / OU" : "Açıklama", "Son Giriş", "Durum"].map((h) => (
          ...
          <div className="divide-y divide-border/40">
            {filtered.map((user) => (
              <div
                key={user.username}
                className="group grid grid-cols-[1fr_1fr_1fr_120px_80px] gap-3 px-3 py-2.5 hover:bg-muted/20 transition-colors items-center"
              >
                <span className="text-[11px] font-mono truncate">{user.username}</span>
                <span className="text-[11px] truncate">{user.displayName}</span>
                <span className="text-[11px] text-muted-foreground truncate">{user.extra || "—"}</span>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="ml-auto hidden group-hover:flex items-center gap-0.5">
                    <button title="Düzenle" className="flex items-center justify-center h-5 w-5 rounded-[3px] text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors">
                      <Pencil className="size-3" />
                    </button>
                    <button title="Şifre Sıfırla" className="flex items-center justify-center h-5 w-5 rounded-[3px] text-muted-foreground hover:text-amber-600 hover:bg-amber-50 transition-colors">
                      <Key className="size-3" />
                    </button>
                    <button title="Sil" className="flex items-center justify-center h-5 w-5 rounded-[3px] text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors">
