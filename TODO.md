...
- [ ] 🔴 **Sayfa kapanınca kurulum yarıda kalıyor (resume/detached job yok)** — `run/route.ts` kurulumu SSE `ReadableStream`'in `start()` içinde çalıştırıyor; `cancel` handler'ı / AbortSignal kontrolü yok. Müşteri sekmeyi kapatır/yenilerse: o an çalışan adım (agent komutu / SQL restore) **bağımsız bittiği için tamamlanır**, ama sonraki `send()` kapalı controller'a yazınca patlar → **kalan adımlar (diğer DB restore, SQL login/mapping, Users.xml, güvenlik kaydı) çalışmaz** → eksik kurulum. Geri açınca kaldığı yeri göstermez, baştan çalıştırmak gerekir (adımlar çoğunlukla idempotent olduğu için toparlar ama riskli). Çözüm: kurulumu tarayıcıdan **bağımsız bir job** olarak çalıştır (DB'de `SetupJobs` durum/adım tablosu) → SSE sadece izleme kanalı olsun, kopsa bile job arka planda sürsün; sayfa geri açılınca job id ile mevcut duruma bağlan (resume/progress replay). Ek: `request.signal` ile gerçek client-abort'ta job'ı **durdurma** seçeneği (kullanıcı bilerek iptal).
